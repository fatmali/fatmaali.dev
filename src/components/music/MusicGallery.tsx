...
export function MusicGallery({ tracks, reverse = false, query, showFilters = true }: MusicGalleryProps) {
  ...
  const playerCardClass = cn(
    "relative overflow-hidden bg-background/90 backdrop-blur",
    reverse && "lg:order-2"
  );
    ...
    <div className="space-y-6 lg:space-y-8">
      ...
      <AnimatePresence mode="popLayout">
        {filteredTracks.length === 0 ? (
          ...
          <motion.div key={currentTrack?.slug ?? "playlist"} layout className="space-y-0">
            {/* Unified Player Container */}
            <div className="rounded-3xl border border-border bg-background/90 backdrop-blur shadow-lg shadow-primary/10 overflow-hidden">
              {/* Top Section: Track Info + Playlist */}
              <section className={cn("grid gap-0", gridCols)}>
                {/* Track Info Panel */}
                <motion.div
                  layout
                ...
                >
                  <div className="sticky top-0 bg-background/95 backdrop-blur z-10 p-5 md:p-6 pb-3 border-b border-border/50">
                  ...
                  <ul className="p-5 md:p-6 pt-3 space-y-3">
